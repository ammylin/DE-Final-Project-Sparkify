...
def generate_listening_events(
    ...
    if "primary_artist" not in songs.columns:
        songs["primary_artist"] = songs["artists"].apply(
            lambda x: ast.literal_eval(x)[0] if len(ast.literal_eval(x)) > 0 else None
        )
    ...
    for genre in set(g for sublist in songs["track_genre_list"] for g in sublist):
        songs_by_genre[genre] = songs[
            songs["track_genre_list"].apply(lambda x: genre in x)
        ]
    ...
    for _, user_row in users.iterrows():
        ...
        for _ in range(events_per_user):
            ...
            track = tracks_df.sample(
                n=1, weights=tracks_df["popularity"] if use_popularity_weights else None
            ).iloc[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="data/synthetic_events.csv")
