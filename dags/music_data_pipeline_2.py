...
pickle_path = os.getenv("MODEL_PICKLE_PATH", "/opt/airflow/data/recommendation_model.pkl")
...
with DAG(
    ...
    def fetch_random_user_and_history():
        ...
        cur.execute("SELECT user_id FROM music_analytics.users ORDER BY RANDOM() LIMIT 1;")
        user_id = cur.fetchone()[0]

        # 2. Fetch history to exclude already listened tracks
        cur.execute("SELECT track_id FROM music_analytics.listening_events WHERE user_id = %s;", (user_id,))
    ...
    def generate_recommendations(user_context: dict, top_k: int = 10):
        ...
        if user_norm == 0: user_norm = 1e-10
