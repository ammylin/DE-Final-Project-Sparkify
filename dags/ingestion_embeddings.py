...
import sys
import json
from recommendation_model import build_track_matrix, generate_user_vector
...
def validate_tracks_df(df: pd.DataFrame):
    ...
    if missing:
        raise ValueError(f"Tracks table missing required columns: {missing}")
    if df.empty:
...
def validate_users_df(df: pd.DataFrame):
    ...
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Users table missing required columns: {missing}")
    if df.empty:
...
def validate_events_df(df: pd.DataFrame):
    ...
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Listening events missing columns: {missing}")
    if df.empty:
        raise ValueError("Listening events table is empty.")
...
def validate_track_embeddings(df: pd.DataFrame):
    ...
    if df["track_embedding"].isnull().any():
...
with DAG(
    ...
    def generate_listening_events(
        events_per_user: int = 15, use_popularity_weights: bool = True
    ):
    ...
    def add_embedding_columns():
        """
        Task 6: Add 64-dim embedding columns to the existing tracks and users tables.
        """
        alter_tables_sql = """
        -- Add vector column to tracks table
        ALTER TABLE music_analytics.tracks
        ADD COLUMN IF NOT EXISTS track_embedding VECTOR(64);

        -- Add vector column to users table
        ALTER TABLE music_analytics.users
        ADD COLUMN IF NOT EXISTS user_embedding VECTOR(64);
        """
        try:
            conn = get_connection()
            cur = conn.cursor()
            cur.execute(alter_tables_sql)
            conn.commit()
            conn.close()
            print("Embedding columns added to tracks and users tables.")
            return "OK"
        except Exception as e:
    ...
    def build_and_save_model_pickle():
        """
        Task 9: Load embeddings, convert to matrix, and save model artifact (PKL).
        """
    ...
    @task
    def validate_tracks_table():
        conn = get_connection()
        df = pd.read_sql("SELECT * FROM music_analytics.tracks;", conn)
        conn.close()
        validate_tracks_df(df)

    @task
    def validate_users_table():
        conn = get_connection()
        df = pd.read_sql("SELECT * FROM music_analytics.users;", conn)
        conn.close()
        validate_users_df(df)

    @task
    def validate_listening_events_table():
        conn = get_connection()
        df = pd.read_sql("SELECT * FROM music_analytics.listening_events;", conn)
        conn.close()
        validate_events_df(df)

    @task
    def validate_track_embeddings_task():
        conn = get_connection()
        df = pd.read_sql(
            "SELECT track_id, track_embedding FROM music_analytics.tracks WHERE track_embedding IS NOT NULL;",
            conn,
        )
        conn.close()
        validate_track_embeddings(df)

    @task
    def validate_user_embeddings_task():
        conn = get_connection()
        df = pd.read_sql(
            "SELECT user_id, user_embedding FROM music_analytics.users WHERE user_embedding IS NOT NULL;",
            conn,
    ...
    create_users_table = generate_users(n_users=500)
